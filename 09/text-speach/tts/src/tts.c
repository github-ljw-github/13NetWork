...
{
	int         ret                  = MSP_SUCCESS;
	const char* login_params         = "appid = 5c947cf5, work_dir = .";
	/*
	* rdn:           合成音频数字发音方式
	* volume:        合成音频的音量
	* pitch:         合成音频的音调
	...
	*/
	...
	void send_wav(int connfd, const char *filename, int size);
	...
	while(1)
	{
		...
		while(1)
		{
			char *text = get_text(connfd);

			if(text == NULL) // 对方断开了套接字
				break;

			printf("文本【%s】===>", strtok(text, "\n"));
			...
			send_wav(connfd, filename, wav_size);
		}
		close(connfd);
		printf("\n\n");
	}
